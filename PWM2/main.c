...
void initPWM() {
    ...
    GPIO_PORTF_AHB_AFSEL_R |= 0x02;   // Enable alternate function on PF0
    GPIO_PORTF_AHB_PCTL_R &= ~0x000000F0;
    GPIO_PORTF_AHB_PCTL_R |= 0x00000060;  // Configure PF0 for PWM (MUX = 6)
    GPIO_PORTF_AHB_DEN_R |= 0x02;    // Enable digital I/O on PF0

    // Configure PWM module 0
    PWM0_0_CTL_R = 0;           // Disable PWM while configuring
    PWM0_0_GENB_R = 0x0000008C;  // Set PWM mode and down-counting
